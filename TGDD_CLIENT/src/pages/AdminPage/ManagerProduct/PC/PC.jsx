...
import FormAddPC from "./FormAddPC";
function PC(props) {
  const { loadingSkeleton } = useSelector((state) => state.global);
  const { pc } = useSelector((state) => state.product);
  ...
  const renderCardList = () => {
    return pc?.map((product, index) => (
      <CardEditCustom key={index} product={product} category={"pc"} />
    ));
  };
  return (
