...
import { GET_SWATCH_SAGA } from "../../../../redux/sagas/types/main";
import AppBarCT from "../global/AppBarCT";
import FormAddSwatch from "./FormAddSwatch";
function Swatch(props) {
  const { loadingSkeleton } = useSelector((state) => state.global);
  const { swatch } = useSelector((state) => state.product);
  ...
  const renderCardList = () => {
    return swatch?.map((product, index) => (
      <CardEditCustom key={index} product={product} category={"swatch"} />
    ));
  };
  return (
    <Box sx={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <Box component="main" sx={{ flex: 1, py: 6, px: 4, bgcolor: "#eaeff1" }}>
        <Paper sx={{ maxWidth: 1000, margin: "auto", overflow: "hidden" }}>
          ...
          <PaginateCT
            length={Math.ceil(
              totalQuantity[0]?.swatch ? totalQuantity[0].swatch / 10 : 0
            )}
            actionName={GET_SWATCH_SAGA}
          />
          <div
            ...
            onClick={() => {
              dispatch({
                ...
                ComponentContent: <FormAddSwatch />,
              });
            }}
          >
