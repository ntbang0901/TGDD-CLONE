...
import { GET_ALL_HISTORY_SIGN_SAGA } from "../../../redux/sagas/types/main";
import Receipt from "./Other/Receipt";
function SignReceipt(props) {
  const { historyAdmin, quantityHistoryAdmin } = useSelector(
    (state) => state.global
  );
  const navigate = useNavigate();
  const { search } = useLocation();
  console.log("qewqewq", queryString.parse(search)?.page);
  ...
  useEffect(() => {
    ...
    dispatch({
      type: GET_ALL_HISTORY_SIGN_SAGA,
    ...
    <>
      ...
      <div className="content px-4 py-2">
        ...
        {historyAdmin?.map((item, index) => (
          <Receipt
            actionName={GET_ALL_HISTORY_SIGN_SAGA}
        ...
        <div className="mt-4 flex justify-center">
          <Pagination
            ...
            onChange={(event, page) => {
              navigate(`/admin/receipt/sign?page=${page}`);
            }}
