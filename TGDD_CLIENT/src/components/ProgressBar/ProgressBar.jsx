...
const ProgressBar = ({text, conditionQuantity, cartQuantity, onClick, notify }) => {
    return (
        <div className={`h-[40px] bg-[#FFF5C2] rounded-xl relative`}>
            <div className={`h-full bg-[#ffd500] rounded-xl px-5 inline-flex transition-all duration-300 items-center si:text-xs`} 
                style={{width: notify ? '100%' : `${cartQuantity/conditionQuantity*100}%`, backgroundColor: notify ? '#9ADE7B' : '#ffd500'}}>
            </div>
